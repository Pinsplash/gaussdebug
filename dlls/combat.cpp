...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// do the damage
	ALERT(at_console, "CBaseMonster Hurting %s named %s for %f damage. (%f -> %f)\n", STRING(pev->classname), STRING(pev->targetname), flDamage, pev->health, pev->health - flDamage);
	pev->health -= flTake;
...
{
	ALERT(at_console, "CBaseMonster DeadTakeDamage Hurting %s named %s for %f damage. (%f -> %f)\n", STRING(pev->classname), STRING(pev->targetname), flDamage, pev->health, pev->health - flDamage);
	Vector			vecDir;
